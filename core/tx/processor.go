...
type Processor interface {
	...
	ProcessEvmTx(evmTx *clients.EvmTransaction) (dbTypes.TxDBWrapper, error)
}
...
func (a *processor) ProcessEvmTx(evmTx *clients.EvmTransaction) (dbTypes.TxDBWrapper, error) {
	...
	message.RawMessage = evmTxJSON

	events := []model.MessageEvent{}

	if evmTx.Value != "0x0" && evmTx.Value != "" {
		transferEvent := model.MessageEvent{
			MessageEventType: model.MessageEventType{
				MessageEventType: "ethereum_tx",
			},
	...
	uniqueEventTypes := make(map[string]model.MessageEventType)
	uniqueEventTypes["ethereum_tx"] = model.MessageEventType{MessageEventType: "ethereum_tx"}
