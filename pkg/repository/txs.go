...
type Txs interface {
	...
	GetVotesByAccounts(ctx context.Context, accounts []string, excludeAccounts bool, voteOption string,
		proposalID int, byAccAddress *string, voteUnique bool, limit int64, offset int64, sortBy *model.SortBy) ([]*model.VotesTransaction, int64, error)
...
func (r *txs) GetWalletsCount(ctx context.Context) (*model.TotalWallets, error) {
	query := `SELECT SUM(unique_wallets_count) FROM daily_wallets_counts where day = date($1)`
	row := r.db.QueryRow(ctx, query, time.Now().UTC())
	var count24H int64
	...
	queryPerDate := `SELECT SUM(unique_wallets_count) FROM daily_wallets_counts where day >= date($1)`
	...
	row = r.db.QueryRow(ctx, queryPerDate, firstDay)
	var count30D int64
	...
	// total wallets
	queryAll := `SELECT SUM(unique_wallets_count) FROM daily_wallets_counts;`
	row = r.db.QueryRow(ctx, queryAll)
	var countAll int64
...
func (r *txs) GetWalletsCountPerPeriod(ctx context.Context, startDate, endDate time.Time) (int64, error) {
	query := `SELECT SUM(unique_wallets_count) FROM daily_wallets_counts where day BETWEEN date($1) and date($2);`
...
func (r *txs) GetVotesByAccounts(ctx context.Context, accounts []string, excludeAccounts bool, voteOption string,
	proposalID int, byAccAddress *string, voteUnique bool, limit int64, offset int64, sortBy *model.SortBy,
	...
	if voteUnique {
		subquery := goqu.Select(goqu.MAX("vn_inner.timestamp")).
			From(goqu.T("votes_normalized").As("vn_inner")).
			Where(goqu.I("vn_inner.voter").Eq(goqu.I("vn.voter"))).
			Where(goqu.I("vn_inner.option").Eq(goqu.I("vn.option"))).
			Where(goqu.I("vn_inner.proposal_id").Eq(goqu.I("vn.proposal_id")))

		dialect = dialect.Where(goqu.I("vn.timestamp").Eq(subquery))
	}
...
func (r *txs) UpdateViews(ctx context.Context) error {
	...
	_, err = r.db.Exec(ctx, `
		REFRESH MATERIALIZED VIEW CONCURRENTLY daily_wallets_counts WITH DATA;
	`)
	if err != nil {
		log.Err(err).Msgf("error refreshing daily_wallets_counts")
		return err
	}
